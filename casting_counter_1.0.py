...
def get_config(config_path, old_config_mtime, config):
    '''checks if config file has beem updated and updates config instance of config_update class'''
    try:
    ...
    if mtime> old_config_mtime:
        try:
            ...
            config = config_default(**configs[0][0])

            # config_source = open(config_path, 'r')
            # config_dict = yaml.load(config_source, Loader)
            # config = config_default(**config_dict)
...
brightness_sum_1 = 0
brightness_sum_2 = 0 
last_part_1_time = datetime.datetime.now()  # use to avoid counting missing part 1 as a part 2


t1_old = datetime.datetime.now()
...
while(True):
    ...
    if mean(conv_deque)<-0.25 and time_toggle == 1:
        ...
        if brightness_sum_1>hsv_thresh:
            made_part_1 = True
            last_part_1_time = datetime.datetime.now()
        else:
            made_part_1 = False
        if hsv_sum_2>hsv_thresh and made_part_1 == False and ((datetime.datetime.now() - last_part_1_time).total_seconds() >=300): # avoids double counting man parts as auto
            made_part_2 = True
        else:
            made_part_2 = False
    ...
    if datetime.datetime.fromtimestamp(cycle_start_time).hour == 1 and config.image_dir_cleanup == False:
        image_directory_cleanup()
    if datetime.datetime.fromtimestamp(cycle_start_time).hour == 2 and config.image_dir_cleanup == True:
